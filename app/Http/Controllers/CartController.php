  ...
  public function deleteCart(Request $request){
	...
	foreach($cart_data as $keys=>$product){
		if($cart_data[$keys]['code']==$request['id']){
			...
			setcookie('code', hash('sha256', $item_data), time()+(86400*30),"/");
			setcookie('shopping_cart', $item_data, time()+(86400*30),"/");
            return redirect('/cart')->with('success', 'The product has been successfully removed from the basket!');
		}
	}
    return redirect('/cart')->with('failed', 'The product failed to be removed from the basket!');
}
